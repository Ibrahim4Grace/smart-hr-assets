(function () {
	...
	if ($('.datatable').length > 0) {
		var table = $('.datatable').DataTable({
			...
			"info": true,
			"paging": true,
			"pagingType": "simple_numbers", // Use simple numbers for easier custom styling
			"language": {
				search: ' ',
			...
			"dom": 't', // Only show the table, hide default pagination/length/info
		});
		...
		function renderCustomPagination(table) {
			...
			html += `<li class="page-item${info.page === 0 ? ' disabled' : ''}">` +
				`<a class="page-link" href="#" aria-label="Previous" data-page="${info.page - 1}">` +
			...
			for (let i = 0; i < info.pages; i++) {
				html += `<li class="page-item${i === info.page ? ' active' : ''}">` +
			...
			html += `<li class="page-item${info.page === info.pages - 1 ? ' disabled' : ''}">` +
				`<a class="page-link" href="#" aria-label="Next" data-page="${info.page + 1}">` +
				`<span aria-hidden="true">&raquo;</span>` +
				`<span class="sr-only">Next</span>` +
				`</a></li>`;

			html += '</ul></nav>';
			$('#todoTablePagination').html(html);
		}
		...
		$('#todoTablePagination').on('click', 'a.page-link', function (e) {
			e.preventDefault();
		...
		// Re-render pagination on table draw
	...
	function init() {
		...
		$('.sidebar-menu a').on('click', function (e) {
			if ($(this).parent().hasClass('submenu')) {
				e.preventDefault();
				// Save the opened submenu to localStorage
				var menuId = $(this).attr('data-menu-id');
				if (!$(this).hasClass('subdrop')) {
					localStorage.setItem('sidebar-open-menu', menuId);
				} else {
		...
		// On page load, restore the open submenu
		var openMenuId = localStorage.getItem('sidebar-open-menu');
		if (openMenuId) {
			var $openMenu = $('.sidebar-menu a[data-menu-id="' + openMenuId + '"]');
			if ($openMenu.length) {
				$openMenu.addClass('subdrop');
				$openMenu.next('ul').show();
			}
		}
